...
real(8)                 ::density(0:1000)
real(8)              ::         statistic(0:1000)
...
com_xycoord(1:3,:)=com_xycoord(1:3,:)/(Natom*0.33)  

do istep=1, Nstep
  do iatom=1, Natom, 3
     coord(1:3,iatom,istep)=coord(1:3,iatom,istep)-com_xycoord(1:3,istep)
  end do
end do
...
do istep=1, Nstep
 do iatom=1, Natom, 3
   do iradius=1,100
     if(sqrt((coord(1,iatom,istep)*coord(1,iatom,istep))+&
             &(coord(3,iatom,istep)*coord(3,iatom,istep))) &
             &.lt. (iradius*0.1) .and. sqrt((coord(1,iatom,istep)*coord(1,iatom,istep))+&
             &(coord(3,iatom,istep)*coord(3,iatom,istep)))&
...
open(58,file="density.dat")
do iradius=1, 100
density(iradius)=((18.0152*statistic(iradius)/6.022E+023)/((((iradius*0.1)*&
&(iradius*0.1))-(((iradius-1)*0.1)*((iradius-1)*0.1)))*3.14*14.32*1.0E-24))
!        write(58,*) 0, 1
        write(58,*) (iradius-1)*0.1, density(iradius)/Nstep
        write(59,*) (iradius-1)*0.1, statistic(iradius)/Nstep
        end do
            ...
            if ((atom_name(iatom) .eq. "O" .and. atom_name(iatom+1) .eq. "H" &
                & .and. atom_name(iatom+2) .eq. "H")) then
                   if(((coord(1,iatom,1)- coord(1,iatom+1,1)) <= 2.0) .and. &
                      &  ((coord(3,iatom,1)- coord(3,iatom+2,1)) <=2.0)) then
