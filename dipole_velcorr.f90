...
!****************************************************************!
integer                 :: Nstep, idum1, idum2, corr
integer                 :: Natom, me,you, hm, idum3 
integer                 :: istep,iatom
...
real(8)                :: angle(1:360,1:360), mittel(1:180), speed(0:20000,0:180)
real(8)                 :: thecount(0:20000,0:180), result_1(0:180), result_2(0:40), thecounttotal(0:180)
real(8), allocatable   :: vektoroh1(:,:,:)
real(8), allocatable   :: vektoroh2(:,:,:)
real(8),allocatable    ::halfvektor(:,:,:)
real(8)                 :: distan1(1:3), result_3(0:180), speed2(0:20000,0:180)
real(8)                 :: distan2(1:3), result_4(0:180)
real(8)                 :: angle_total,highest, speedave, counttotal
real(8)                 ::speedxtotal(0:180), counthighest, speedhighest
...
allocate(vel(1:3,1:Natom,1:Nstep))
...
!call calculate_angle(Noh, hcoord,ocoord,Nstep,h2o_atoms,alpha)


do iatom=1, Natom,2
do istep = 1, Nstep-1
!        print*, h2o_coord(1:3,iatom,Nstep-1), h2o_coord(1:3,iatom,Nstep)
        vel(1:3,iatom,istep)=((ocoord(1:3,iatom,istep+1)-ocoord(1:3,iatom,istep))/dt)
...
angle2(iatom,istep)=abs(angle2(iatom,istep)*57.53)
...
do istep=1, Nstep-20001,10
idum1=1
!idum2=1
do iatom=1, Noh,2
!speedave=speedave+sqrt(vel(2,idum1,istep)*vel(2,idum1,istep))
do you=1, 180
if(angle2(iatom,istep) .ge. (you-1) .and. angle2(iatom,istep) .lt. (you+1) .and. &
        &sqrt((ocoord(1,iatom,istep)*ocoord(1,iatom,istep))+(ocoord(3,iatom,istep)*&
        &ocoord(3,iatom,istep))) .ge. 0.0 .and. &
        &sqrt((ocoord(1,iatom,istep)*ocoord(1,iatom,istep))+(ocoord(3,iatom,istep)*&
        &ocoord(3,iatom,istep))) .lt. 8.0)then
                   do corr=1, 19999, 1
                        speed(corr,you-1)=speed(corr,you-1)+((ocoord(2,iatom,istep+corr)-ocoord(2,iatom,istep))*&
                                &(ocoord(2,iatom,istep+corr)-ocoord(2,iatom,istep)))
                        !                        speed2(corr,you-1)=speed(corr,you-1)+(vel(1,idum1,istep+corr)*vel(1,idum1,istep+corr))+&
                                !        &(vel(3,idum1,istep+corr)*vel(3,idum1,istep+corr))
!                        print*, speed(you-1), speed2(you-1)
!                        print*, vel(2,idum1,istep+corr), vel(3,idum1,istep+corr), vel(1,idum1,istep+corr)
                        thecount(corr,you-1)=thecount(corr,you-1)+1
                        !print*, speed(corr,you-1)
                   end do
!                   if(thecount(corr,you-1) .gt. 0) then
!                        speed(corr,you-1)=speed(corr,you-1)/thecount(corr,you-1)  
!                        speed2(corr,you-1)=speed2(corr,you-1)/thecount(corr,you-1)  
!                   end if
!        do me=1, 180
!        if(angle2(iatom+1,istep) .ge. (me-1) .and. angle2(iatom+1,istep) .lt. (me) .and. &
!                &sqrt((ocoord(1,iatom+1,istep)*ocoord(1,iatom+1,istep))+(ocoord(3,iatom+1,istep)*&
!                &ocoord(3,iatom+1,istep))) .ge. 4.0 .and. &
!                &sqrt((ocoord(1,iatom+1,istep)*ocoord(1,iatom+1,istep))+(ocoord(3,iatom+1,istep)*&
!                &ocoord(3,iatom+1,istep))) .lt. 8.0)then
!                angle(you,me)=angle(you,me)+1
!                angle_total=angle_total+angle(you,me)
!        endif   
!        end do           
end if
end do
...
!        speed(corr,you-1)=speed(corr,you-1)/thecount(corr,you-1)  
!        speed2(corr,you-1)=speed2(corr,you-1)/thecount(corr,you-1)  
        thecounttotal(you-1)=thecounttotal(you-1)+thecount(corr,you-1)
end if
end do 
end do

print*, speed(100,100), speed(1900,100)

counthighest=0
speedhighest=0
do you=1, 180
     speedxtotal(you-1)=(speed(19999,you-1)-speed(9999,you-1)/10000)*1E-05
     if(speedhighest .lt. (speedxtotal(you-1))/thecounttotal(you-1)) then
             speedhighest=speedxtotal(you-1)/thecounttotal(you-1)
     end if
     if(counthighest .lt. thecounttotal(you-1))then
             counthighest=thecounttotal(you-1)
     end if
end do 
...
!calculation of OH/OH angle distribution was done before. here its written into files.
...
!do iatom=1, Noh,2
!do you=1, 180
!if(angle2(iatom,istep) .ge. (you-1) .and. angle2(iatom,istep) .lt. (you+1)) then
!        mittel(you)=mittel(you)+1
!end if
!end do
!end do
!end do
...
!speedave=0
!do idum1=1,180
!     if(thecount(idum1-1) .gt. 0) then
!     counttotal=counttotal+thecount(idum1-1)
!     result_1(idum1-1)=speed(idum1-1)/(thecount(idum1-1)*2)
!     result_3(idum1-1)=speed2(idum1-1)/(thecount(idum1-1)*4)
!     print*,result_1(idum1-1)
!!     speedave=speedave+result_1(idum1-1)
!     end if
!     end do
!counttotal=counttotal/180
!speedave=speedave/180 
!print*,"speed:",speedave

open(61, file="speednew.dat")
do you=1,180
        write(61,*), you-1, (speedxtotal(you-1)/thecounttotal(you-1))/speedhighest, thecounttotal(you-1)/counthighest,&
                &speedxtotal(you-1)/thecounttotal(you-1)
end do 
close(61)
!do idum2=1,180
!      angle(idum1,idum2)=angle(idum1,idum2)/highest
!      write(61,*) ((idum1)-1), ((idum2)-1), angle(idum1,idum2)     
!      end do
!      end do
!close(61)


!open(62, file="distribution.dat")
!do idum1=1,180
!        write(62,*) idum1-1, result_1(idum1-1), result_3(idum1-1)
!        end do
!close(62)
!idum3=0
!idum2=0
!do idum1=1,180
!        result_2(idum2)=result_2(idum2)+result_1(idum1-1)
!        thecount_2(idum2)=thecount_2(idum2)+thecount(idum1-1)
!        result_4(idum2)=result_4(idum2)+result_3(idum1-1)
!        idum3=idum3+1
!        if(idum3==5) then
!                idum2=idum2+1
!                idum3=0
!        end if
!        end do
!open(63, file="part.dat")
!do hm=1,36
!        write(63,*) hm-1, result_2(hm-1), result_4(hm-1)
!        end do
!close(63)

!open(64, file="dipole.dat")
!do idum1=1,180
!        write(64,*) idum1-1, thecount(idum1-1)/counttotal, (idum1-1)*5, thecount_2(idum1-1)/(counttotal*5)
!        end do        
!close(64)
